...
public class IndicadorDbImpl implements IndicadorDb, WithGlobalEntityManager, TransactionalOps {
	...
	public String obtenerFormula(String nombre) throws ArchivoException {
		try {
			return entityManager().createQuery("from Indicador i WHERE i.nombre LIKE :nombreX", Indicador.class)
					.setParameter("nombreX", nombre).setMaxResults(1).getSingleResult().getFormula();
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		return null;
