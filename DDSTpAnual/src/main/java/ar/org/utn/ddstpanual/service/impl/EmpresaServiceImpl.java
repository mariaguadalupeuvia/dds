...
public class EmpresaServiceImpl implements EmpresaService {
  ...
  public List<Empresa> obtenerEmpresas() throws ServiceException {
	...
	@Override
	public List<EmpresaExcel> buscar(final Empresa empresa, final Cuenta cuenta, final Periodo periodo)
			throws ServiceException {
		List<EmpresaExcel> empresas = new ArrayList<EmpresaExcel>();
		if (cuenta != null) {
			final List<Cuenta> cuentas = empresa.getCuentas().stream().filter(c -> c.getNombre().equals(cuenta.getNombre()))
					.collect(Collectors.toList());
			empresa.setCuentas(cuentas);
		}
		if (periodo != null) {
			final List<Periodo> periodos = cuenta.getPeriodos().stream().filter(p -> p.getFecha().equals(periodo.getFecha()))
					.collect(Collectors.toList());
	...
	public List<Periodo> obtenerPeriodos() throws ServiceException {
		try {
			return getEmpresaDb().obtenerPeriodos("Facebook");
